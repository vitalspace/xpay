...
export const connectWallet = async () => {
  await kit.openModal({
    ...
    onWalletSelected: (option: ISupportedWallet) => {
      ...
      void kit.getAddress().then(async (address) => {
        ...
        if (address.address) {
          ...
          // Send request to backend when wallet is connected
          console.log("Wallet connected, sending create user request for address:", address.address);
          try {
            await userService.createUser({ address: address.address });
